...
function displaySelectedMenu() {
    ...
    while (secondMenu.firstChild) {
        secondMenu.removeChild(secondMenu.firstChild);
      }
...
if (category.value == 0){
    for (let i = 0; i < 3; i++) {
        ...
        theOption.textContent = activities[i].name;
        theOption.value = i;

        secondMenu.appendChild(theOption);
      }
}
...
function displayActivity(){
    ...
    const outputParagraph = document.getElementById("activity");
    ...
    //   }
      ...
      let values = secondMenu.value 

      outputTitle.innerText = activities[values].name;
      outputParagraph.innerText = activities[values].description;
      outputPrice.innerText = "$" + activities[values].price;
}