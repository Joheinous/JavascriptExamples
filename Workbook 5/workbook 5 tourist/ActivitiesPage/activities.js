...
function buildActivityRow(tbody, theActivity) {
    let row = tbody.insertRow(-1);

    let cell1 = row.insertCell(0);
    cell1.innerText = theActivity.name;

    let cell2 = row.insertCell(1);
    cell2.innerText = "$" + theActivity.price.toFixed(2);

    let cell3 = row.insertCell(2);
    cell3.innerText = theActivity.location;

    let cell4 = row.insertCell(3);
    cell4.innerText = theActivity.description;
  }
...
if (category.value == 0){
    for (let i = 0; i < 3; i++) {
        ...
        buildActivityRow(output, activities[i]);
      }
}
